$(function() {
  ...
  test('triggerDesktopNotification', function() {
    expect(4);
    var old_getSetting = getSetting;
    var old_chromeNotifications = chrome.notifications;
    ...
    getSetting = function(name) {
      ok(name == SETTINGS.notifications_enabled, 'Valid setting requested.');
      return false;
    };
    ...
    chrome.notifications = {
      create: function(id, options, callback) {
        same(options, {
          ...
          title: '2 Pages Updated',
          message: '',
          buttons: [{title: 'foo'}, {title: 'bar'}]
        }, 'Notification page');
        callback(id);
      },
      onButtonClicked: {
    ...
    chrome.notifications = old_chromeNotifications;
    getSetting = old_getSetting;
  ...
  test('check', function() {
    ...
    $ = {
      ajax: function(args) {
        ...
        args.complete({ status: 200,  responseText: 'Google' });
      }
    };
