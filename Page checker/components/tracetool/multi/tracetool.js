...
if (IsNodeJs)    
{
    ...
    var ttraceScript = null;                                /** current trace script              */
    var headId = global.document.getElementsByTagName("head")[0];  /** Shortcut to head. used by RunScript() */
...
function detectEnvironment() 
{
    ...
    var typeofChrome = typeof chrome;        // chrome browser or chrome browser extension : 'object'
    var typeofChromeExtension;                 // chrome browser extension : 'object'
    
    if (typeofChrome === 'object')
        typeofChromeExtension = typeof chrome.extension;
    
    if (typeofChromeExtension === 'object') {
        IsChromeExtension = true;
        return;
    }
    
    var typeofRequire = typeof require;       // nodejs : 'function'
    var typeofProcess = typeof process;      // nodejs : 'object'
...
function worker()
{
    ...
    if (ToSend.length !== 0)
    {
        ...
        NbDone++;
        if (IsNodeJs)
            sendToClientUsingRequest(objMessage);
        else if (IsChromeExtension)
            sendToClientUsingXMLHttpRequest(objMessage);
        else 
            sendToClientUsingScript(objMessage);
    }
}
 ...
 * @returns {void}
 */
function sendToClientUsingScript(objMessage)
{
...
function afterRun(msgId, partNum) 
{
    ...
    //    sendToClientUsingScript(objMessage);          // run the script
    //
};
...
* @returns {void}
*/  
function sendToClientUsingRequest (objMessage)
{
   var hostUrl = "http://" + Host + "/" + objMessage.command + "?msgId=" + objMessage.msgId  + "&msg=" + escape(objMessage.msg) ;  // '&method=script'  + '&crc=' + objMessage.crc
   if (objMessage.partNum !== '')
      hostUrl = hostUrl + '&partNum=' + objMessage.partNum ;
